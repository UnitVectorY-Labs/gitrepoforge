...
parent: Reference
nav_order: 3
permalink: /reference/git-operations
---

# Git Operations

gitrepoforge performs Git operations during `apply` when an action is selected with `--action`. All operations run inside each target repository using the local `git` CLI and, for pull requests, the GitHub CLI (`gh`). The behavior of each step is controlled by the Git fields defined in the named action under the `action` section of the workspace config (see [Workspace Config](/configuration/workspace)).

If no `--action` flag is provided, `apply` does not write files and does not run any Git commands; it only reports drift.
