...
managed_files:
  - path: .gitrepoforge-managed-files.yaml
...
| `managed_files[].management` | `whole_file` when the full file is replaced, or `sections` when only specific regions are managed. |
...
- Whole-file outputs are listed as `management: whole_file`.
