...
| `--version`, `-v` | top-level | Print version as `gitrepoforge version vX.Y.Z (goX.Y, os/arch)` where `os/arch` is the target platform (for example, `linux/amd64` or `darwin/arm64`). |
